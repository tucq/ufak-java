...
public class ProductCategoryServiceImpl extends ServiceImpl<ProductCategoryMapper, ProductCategory> implements IProductCategoryService {
	...
	public void addProductCategory(ProductCategory productCategory) {
		...
		if(oConvertUtils.isNotEmpty(productCategory.getPid())){
			categoryPid = productCategory.getPid();

			//PID 不是根节点 说明需要设置父节点 hasChild 为1
			if(!IProductCategoryService.ROOT_PID_VALUE.equals(categoryPid)){
				ProductCategory parent = baseMapper.selectById(categoryPid);
	...
	public void updateProductCategory(ProductCategory productCategory) {
		if(oConvertUtils.isEmpty(productCategory.getPid())){
			productCategory.setPid(IProductCategoryService.ROOT_PID_VALUE);
		}else{
			//如果当前节点父ID不为空 则设置父节点的hasChild 为1
			ProductCategory parent = baseMapper.selectById(productCategory.getPid());
			if(parent!=null && !"1".equals(parent.getHasChild())){
				parent.setHasChild("1");
				baseMapper.updateById(parent);
			}
	...
	public List<TreeSelectModel> queryListByCode(String pcode) throws JeecgBootException{
		...
		if(oConvertUtils.isNotEmpty(pcode)) {
			List<ProductCategory> list = baseMapper.selectList(new LambdaQueryWrapper<ProductCategory>().eq(ProductCategory::getCode, pcode));
